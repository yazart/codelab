...
export class EditorContentManager {
   ...
   * @internal
   */
  private _onContentChanged = (e: monaco.editor.IModelContentChangedEvent) => {
    if (!this._suppress) {
      e.changes.forEach((change: monaco.editor.IModelContentChange) => this._processChange(change));
    }
  }
   ...
   * @internal
   */
  private _processChange(change: monaco.editor.IModelContentChange): void {
