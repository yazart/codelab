...
export class RemoteSelectionManager {
  ...
  public addSelection(id: string, color: string, label?: string): RemoteSelection {
    ...
    const selection = new RemoteSelection(this._options.editor, id, this._nextClassId++, color, label || 'label', onDisposed);
  ...
  /**
   * Sets the selection using the Monaco Editor's monaco.IPosition (line numbers and columns)
   * location concept.
   *
   * @param id
   *   The unique id of the selection.
   ...
   */
  public setSelectionPositions(id: string, start: monaco.IPosition, end: monaco.IPosition): void {
    const remoteSelection = this._getSelection(id);
  ...
  private _getSelection(id: string): RemoteSelection {
    const selection = this._remoteSelections.get(id);
    if (!selection) {
